...
fs.copyFileSync(file, `${file}.bak_home_buttons_inside_menu_v2_${stamp}`);

let text = fs.readFileSync(file, "utf8");

// Quitar bloques anteriores de Historial / Salir que pudieron quedar fuera del inicio.
text = text.replace(
  /\s*<div className="home-session-actions[\s\S]*?<button[\s\S]*?<\/button>\s*<button[\s\S]*?<\/button>\s*<\/div>\s*/g,
  "\n"
);
...
if (!block.includes("home-session-actions-inline")) {
  const sectionClose = block.lastIndexOf("</section>");

  if (sectionClose < 0) {
    throw new Error("No encontr? </section> dentro de WalletActionsPanel.");
  }
...
`;
  ...
  block = block.slice(0, sectionClose) + buttons + block.slice(sectionClose);
}
...
console.log("Botones Historial / Salir insertados correctamente dentro del men? principal.");
