...
const statusLine = "  const [status, setStatus] = useState('');";

if (!text.includes(statusLine)) {
    throw new Error("No encontre estado status del modal.");
}

function addState(line) {
    if (!text.includes(line)) {
        text = text.replace(statusLine, statusLine + "\n" + line);
    }
}

addState("  const [depositSourceWallet, setDepositSourceWallet] = useState('');");
addState("  const [depositDetectedNetwork, setDepositDetectedNetwork] = useState('');");
addState("  const [depositDetectedLabel, setDepositDetectedLabel] = useState('');");
addState("  const [depositDetectStatus, setDepositDetectStatus] = useState<'idle' | 'valid' | 'warning' | 'invalid'>('idle');");
addState("  const [depositDetectMessage, setDepositDetectMessage] = useState('');");
addState("  const [depositIntent, setDepositIntent] = useState<any | null>(null);");

if (!text.includes("function detectDepositNetworkFromWallet")) {
    const detector = `  function detectDepositNetworkFromWallet(rawWallet: string) {
...
`;
    ...
    if (!text.includes("  async function postJson(url: string, payload: any) {")) {
        throw new Error("No encontre postJson para insertar detector.");
    }

    text = text.replace(
        "  async function postJson(url: string, payload: any) {",
        detector + "  async function postJson(url: string, payload: any) {"
    );
}
...
if (start === -1 || end === -1 || end <= start) {
    throw new Error("No pude ubicar el bloque submit de deposit.");
}
...
if (text.includes(oldDepositForm)) {
    text = text.replace(oldDepositForm, newDepositForm);
} else {
    const formStart = text.indexOf("        {action === 'deposit' && (");
    const formEnd = text.indexOf("        {action === 'sell-p2p'", formStart);

    if (formStart === -1 || formEnd === -1 || formEnd <= formStart) {
        throw new Error("No pude ubicar formulario de deposit.");
    }

    text = text.slice(0, formStart) + newDepositForm + "\n\n" + text.slice(formEnd);
}
...
const result = {
    ...
    hasDuplicateGuard: after.includes("No se generara otra wallet"),
    noGlobalDisabledBug: !after.includes("quickLogin} disabled={loading || (action === 'deposit'")
};
