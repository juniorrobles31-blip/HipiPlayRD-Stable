...
function hipiSanitizeDepositMessage(value: unknown): string {
  let msg = String(value || "").trim();

  if (!msg) return "";

  msg = msg.replace(/Wallet de pago generada\\.?/gi, "Recarga creada.");
  msg = msg.replace(/Wallet destino generada\\.?/gi, "Recarga creada.");
  msg = msg.replace(/Wallet origen:\\s*\\n?0x[a-fA-F0-9]{40}\\s*/gi, "");
  msg = msg.replace(/Tu wallet:\\s*\\n?0x[a-fA-F0-9]{40}\\s*/gi, "");
  ...
  msg = msg.replace(/Estado:\\s*Pendiente de pago/gi, "Estado: Pendiente de pago");
  msg = msg.replace(/Estado:\\s*PAID/gi, "Estado: Pago confirmado");

  msg = msg.replace(/Enviar\\s+USDT\\s+a\\s+esta\\s+wallet\\s+destino:/gi, "Dirección de pago:");
...
/*
  Wallet destino completa nunca visible.
*/
text = text.replace(
  /<code([^>]*)>\s*\{\s*depositIntent\.depositAddress\s*\}\s*<\/code>/g,
  `<button
    type="button"
...
/*
  Sanitizar bloque grande técnico.
*/
text = text.replace(
  /\{\s*depositStatusMessage\s*\}/g,
  "{hipiSanitizeDepositMessage(depositStatusMessage)}"
);

/*
  Agregar botón de copiar en bloque de recarga creada.
*/
text = text.replace(
  /<h3>\s*Recarga creada\s*<\/h3>/g,
...
/*
  Evitar duplicar botón si se ejecuta más de una vez.
*/
text = text.replace(
  /(\{depositIntent \? \(\s*<button[\s\S]*?hipiMaskWallet\(hipiDepositAddressFromIntent\(depositIntent\), 8\)\}[\s\S]*?<\/button>\s*\) : null\}\s*){2,}/g,
  "$1"
);
...
console.log(JSON.stringify({
  ...
  statusSanitized: text.includes("hipiPaymentStatus"),
  messageSanitized: text.includes("hipiSanitizeDepositMessage(depositStatusMessage)"),
  copyButton: text.includes("hipi-copy-address-btn"),
  badVisible
}, null, 2));

if (badVisible.length > 0) {
  throw new Error("Quedan textos técnicos visibles: " + badVisible.join(", "));
}