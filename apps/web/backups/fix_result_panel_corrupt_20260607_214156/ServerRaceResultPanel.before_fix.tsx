...
export function ServerRaceResultPanel({
  ...
  const hasFallbackTicket =
    !serverPlayerResult &&
    typeof selectedHorse === 'number' &&
    selectedHorse > 0 &&
    Number(betAmount || 0) > 0;

  const hasTicket = hasServerTicket || hasFallbackTicket;
  const won = Boolean(hasServerTicket && serverPlayerResult?.won);
  const payout = hasServerTicket ? Number(serverPlayerResult?.totalPayout || 0) : 0;
  const totalBet = hasServerTicket ? Number(serverPlayerResult?.totalBetAmount || 0) : Number(betAmount || 0);
      ...
      <div className="result-panel-compact server-results-panel">
        ...
        <div className={'result-coins-box ' + resultClass}>
          ...
          {!hasTicket ? (
            <>
              <strong>0</strong>
              <span>No generserver-result-waiting">
            Esperando resultado oficial del servidor...
          </div>
        )}

        <div className={'result-coins-box ' + resultClass}>
          <aste boleto en esta carrera.</span>
            </>
          ) : won ? (
            <>
              <strong>+{coins(payout)}</strong>
              <span>
                Ganaste. Apostaste {coins(totalBet)} y el servidor pagó {coins(payout)} monedas.
              </span>
            </>
          ) : (
            <>
              <strong>0</strong>
              <span>
                Perdiste. El monto apostado ya fue descontado de tus monedas.
              </span>
            </>
          )}
        </div>
