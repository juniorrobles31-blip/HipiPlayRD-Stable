...
useEffect(() => {
  ...
  async function applyLocalWalletDelta(delta: number, movementId: string) {
    if (!Number.isFinite(delta) || delta === 0) return;

    const movementKey = 'hipiplay_wallet_movement_' + movementId;
    const storageKey = 'hipiplay_wallet_' + user.id;

    if (localStorage.getItem(movementKey)) return;

    let storedWallet: Partial<LocalWalletState> | null = null;

    try {
      const raw = localStorage.getItem(storageKey);
      storedWallet = raw ? JSON.parse(raw) as Partial<LocalWalletState> : null;
    } catch {
      storedWallet = null;
    }

    const baseWallet = storedWallet || wallet || { userId: user.id, demoBalance: 0 };
         storedWallet = raw ? JSON.parse(raw) as Partial<LocalWalletState> : null;
    } catch {
      stored const currentBalance = Number(baseWallet.demoBalance || 0);
    const nextBalance = Math.max(0, Math.floor(currentBalance + delta));
    ...
    try {
      ...
      localStorage.setItem(movementKey, '1');
    } catch {
      // No interrumpir la app si localStorage falla
    }
  ...
  async function bet() {
    ...
    try {
      ...
      await applyLocalWalletDelta(-safeAmount, betId);
