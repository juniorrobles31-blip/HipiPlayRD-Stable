...
import { getServerRaceState, ServerRaceState } from './hipiplayServerApi';
...
function DerbyGame({ user, wallet, refreshLocal }: { user: User; wallet: LocalWalletState | null; refreshLocal: () => Promise<void> }) {
  ...
  const bettingOpen = (serverRaceState ? serverRaceState.phase === 'BETTING' : cycle.phase === 'betting') && !currentUserBet && getMarketStatus(currentExposure).canAcceptBet;
...
useEffect(() => {
  ...
  async function bet() {
    ...
    try {
      ...
      setMessage(`Boleto generado: ${res.bet.id}. Espera el cierre de apuestas; la carrera iniciará pronto.`);
      await refreshLocal();
      await loadLocalBets();
      syncNowQuietly();
    } catch (err) {
...
<div className="cycle-steps glass">
      ...
      <span>
        {serverOnline && serverRaceState
          ? `Ronda ${serverRaceState.roundId} · ${serverRaceState.phase === 'BETTING' ? 'Apuestas abiertas' : 'Carrera en curso'} · ${serverRaceState.secondsRemaining}s`
...
{serverRaceState ? (
      serverRaceState.phase === 'BETTING' ? (
        ...
        <RaceVideoEngine
          key={`server-${serverRaceState.roundId}-${serverRaceState.phase}`}
          raceId={`server-round-${serverRaceState.roundId}`}
          winners={
            serverRaceState.winners && serverRaceState.winners.length >= 3
              ? [...serverRaceState.winners]
                  .sort((a, b) => a.position - b.position)
                  .map((winner) => winner.horseId)
                  .slice(0, 3)
              : resultOrder.slice(0, 3)
          }
          selectedHorse={currentUserBet?.selectedHorse || horse}
          betAmount={currentUserBet?.amount || amount}
          autoPlay
          startAtSeconds={
            serverRaceState.phase === 'RESULT'
              ? serverRaceState.raceSeconds || 20
              : Math.max(0, (serverRaceState.raceSeconds || 20) - serverRaceState.secondsRemaining)
          }
          onFinish={() => {
            console.log('Carrera sincronizada con servidor terminó:', serverRaceState.roundId);
          }}
        />
      )
    ) : videoRaceSession ? (
