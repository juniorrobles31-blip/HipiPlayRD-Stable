...
function PlayerSummary({ wallet }: { wallet: LocalWalletState | null; pendingCount: number; lastBet?: LocalDerbyBet; lastResult?: RaceResult | null }) {
  return <section className="coin-dashboard coin-dashboard-single">
    <div className="coin-card featured hipiplay-secondary-balance-card">
      ...
      <strong><WalletBalanceValue wallet={wallet} /></strong>
    </div>
  </section>;
}
...
function WalletBalanceValue({ wallet }: { wallet: LocalWalletState | null }) {
  ...
  useEffect(() => {
    ...
    async function loadServerPlayerResultAndApplyBalance() {
      try {
        ...
        const finalBalance = Number(result.finalBalance);
        const resultRound = result.roundId || result.raceNumber || serverRaceState.roundId;
        const resultKey = String(resultRound) + '-' + result.playerId + '-' + String(finalBalance);
...
useEffect(() => {
  ...
  }, [cycle.cycleIndex, cycle.phase]);  async function applyServerWalletBalance(nextBalanceRaw: number) {
  ...
  async function bet() {
    ...
    const balanceTotal = Math.floor(Number(wallet?.demoBalance || 0));
    ...
    try {
      ...
      const balanceAfterBet = Number(res.bet?.balanceAfterBet);
