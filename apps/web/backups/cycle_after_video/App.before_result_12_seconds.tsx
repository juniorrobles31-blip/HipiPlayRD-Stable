...
const RESULT_HOLD_AFTER_VIDEO_MS = 5_000;
  ...
  return <div className="player-screen">
    ...
    <section className="game-hero glass cycle-hero">
      <div className="game-title-block">
        <div className="big-icon brand-logo-tile"><img src={hipiPlayLogo} alt="HipiPlay" className="hipiplay-logo hero-logo" /></div>
        <div>
          <h2>HipiPlay</h2>
          <p>60 segundos para apostar, 60 segundos de carrera y 10 segundos de resultado oficial en una experiencia hípica pensada para móvil.</p>
        </div>
      </div>
      <div className="cycle-status">
        <span className={videoRaceSession && !videoRaceSession.finished ? 'live-dot racing' : 'live-dot'}></span>
        <strong>{videoRaceSession ? (videoRaceSession.finished ? 'Resultado oficial' : 'Carrera en vivo') : cycle.phase === 'betting' ? 'Apuestas abiertas' : cycle.phase === 'running' ? 'Carrera en vivo' : 'Resultado oficial'}</strong>
    ...
    <div className="cycle-steps glass">
      ...
      <div className={cycle.phase === 'running' ? 'step active' : 'step'}><span>02</span><strong>Carrera</strong><small>Video completo</small></div>
      <div className="step-line"></div>
    ...
    <div className="bet-panel glass">
      ...
      <button className="primary bet-button" onClick={bet} disabled={!bettingOpen || loading}>
        {loading ? 'Generando boleto...' : currentUserBet ? 'Boleto generado' : cycle.phase !== 'betting' ? 'Apuestas cerradas' : `Apostar ${coins(amount)} monedas al #${horse}`}
      </button>
    </div>

    {videoRaceSession ? (
      <RaceVideoEngine
        key={videoRaceSession.raceId}
        raceId={videoRaceSession.raceId}
        winners={videoRaceSession.winners}
        autoPlay
        onFinish={() => {
          console.log('RaceVideoEngine terminó:', videoRaceSession.raceId, videoRaceSession.winners);

          setVideoRaceSession((current) =>
            current && current.raceId === videoRaceSession.raceId
              ? { ...current, finished: true }
              : current
          );

          if (resultHoldTimerRef.current) {
            window.clearTimeout(resultHoldTimerRef.current);
          }

          resultHoldTimerRef.current = window.setTimeout(() => {
            const sessionRaceIndex = Number(videoRaceSession.raceId.replace('race-', ''));
            const nextCycleIndex = Number.isFinite(sessionRaceIndex)
              ? sessionRaceIndex + 1
              : cycle.cycleIndex + 1;

            sessionEpochRef.current = Date.now() - nextCycleIndex * CYCLE_WINDOW_MS;

            setNow(Date.now());
            setVideoRaceSession(null);
            resultHoldTimerRef.current = null;
          }, RESULT_HOLD_AFTER_VIDEO_MS);
        }}
      />
    ) : cycle.phase === 'betting' ? (
      <section className="race-waiting-panel glass">
        <div className="race-waiting-kicker">Próxima carrera</div>

        <h2>Apuestas abiertas</h2>

        <p>
          Selecciona tu caballo y confirma tu boleto antes de que cierre el contador.
          La carrera visual iniciará automáticamente cuando termine el tiempo de apuestas.
        </p>

        <div className="race-waiting-grid">
          <div>
            <span>Tiempo restante</span>
            <strong>{Math.ceil(cycle.phaseRemaining / 1000)}s</strong>
          </div>

          <div>
            <span>Carrera</span>
            <strong>{cycle.raceCode}</strong>
          </div>

          <div>
            <span>Estado</span>
            <strong>{currentUserBet ? `Boleto #${currentUserBet.selectedHorse}` : 'Sin boleto activo'}</strong>
          </div>
        </div>
      </section>
    ) : (
      <RaceTrack
        ...
        selectedHorse={currentUserBet?.selectedHorse || horse}
...
export function App() {
  ...
  return <div className="app-shell clean-player-shell">
    ...
    <header className="topbar glass clean-topbar">
      ...
      <nav>
        <button className={tab === 'games' ? 'active' : ''} onClick={() => setTab('games')}><Trophy size={17}/> HipiPlay</button>
