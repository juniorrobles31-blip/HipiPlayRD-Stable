    ...
    async function loadServerPlayerResultAndApplyPayout() {
      try {
        ...
        for (const settlement of settlements) {
          if (settlement?.betId && !chargedServerBetIdsRef.current.has(settlement.betId)) {
            chargedServerBetIdsRef.current.add(settlement.betId);
            applyWalletDelta(-Number(settlement.amount || 0));
          }
        }

        const resultRound = result.roundId || result.raceNumber || serverRaceState?.roundId || 0;
        const resultKey = String(resultRound) + '-' + result.playerId;
