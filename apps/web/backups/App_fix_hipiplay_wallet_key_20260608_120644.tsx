    ...
    <div className="login-card glass clean-login-card">
      <div className="login-brand-lockup">
        ...
        <div>
          ...
          <h1>HipiPlay</h1>
          <p>Carreras HÃ­picas con monedas. Apuesta, espera la salida y vive una carrera automÃ¡tica de 60 segundos.</p>
        </div>
      </div>
...
function PlayerSummary({ wallet, pendingCount, lastBet, lastResult }: { wallet: LocalWalletState | null; pendingCount: number; lastBet?: LocalDerbyBet; lastResult?: RaceResult | null }) {
  ...
  return (
    <section className="player-summary glass">
      <div className="summary-card wallet-dynamic-card">
        <small>Monedas disponibles</small>
        <strong>{coins(displayBalance)}</strong>
        <span>Balance actualizado por el servidor</span>
      </div>

      <div className="summary-card">
        <small>Apuestas pendientes</small>
        <strong>{pendingCount}</strong>
        <span>{lastBet ? `Última apuesta: #${lastBet.selectedHorse}` : 'Sin apuestas pendientes'}</span>
      </div>

      <div className="summary-card">
        <small>Último resultado</small>
        <strong>{lastResult ? (lastResult.won ? 'Ganó' : 'Perdió') : '-'}</strong>
        <span>{lastResult?.raceCode || 'Esperando carrera'}</span>
      </div>
    </section>
  );
}
...
useEffect(() => {
  ...
  async function bet() {
    ...
    let balanceTotal = Math.floor(Number(wallet?.demoBalance || 0));

    try {
      const storedWalletRaw = localStorage.getItem(hipiplay_wallet_);
      ...
      if (storedWalletRaw) {
        ...
        if (Number.isFinite(storedBalance)) {
          balanceTotal = Math.max(0, Math.floor(storedBalance));
        }
      }
    } catch {
      // Si localStorage falla, se usa el balance recibido por props.
    }
    ...
    if (!Number.isFinite(balanceTotal) || balanceTotal < safeAmount) {
