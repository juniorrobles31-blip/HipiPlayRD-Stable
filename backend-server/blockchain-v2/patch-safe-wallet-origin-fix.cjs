...
if (!text.includes("HIPIPLAY_PUBLIC_METADATA_AFTER_CREATE")) {
    const needle = `source: "PWA"
            });`;

    if (!text.includes(needle)) {
        throw new Error("No encontre el bloque public createIntent con source PWA.");
    }
    ...
    text = text.replace(
        ...
        // HIPIPLAY_PUBLIC_METADATA_AFTER_CREATE
        const hipiPublicMetadata =
...
const result = {
    ...
    hasPublicMetadataHook: after.includes("HIPIPLAY_PUBLIC_METADATA_AFTER_CREATE"),
    ...
    noCreateWrapper: !after.includes("function createIntentWithMetadata")
};
